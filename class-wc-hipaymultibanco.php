...
function woocommerce_hipaymultibanco_init() {
	...
	class WC_HipayMultibanco extends WC_Payment_Gateway  {
		...
		public function admin_options() {
			...
			<p><?php _e('Emita Refer&ecirc;ncias Multibanco na sua loja online, pagas na rede Multibanco ou atrav&eacute;s de HomeBanking.', $this->id); ?></p>
			...
			<tr>
				...
				<td>
                                        <?php
					if (!$soap_active) echo __( 'Instale ou ative a livraria SOAP para funcionamento correto.', $this->id );       ?>
				</td>
			</tr>
				...
				<td class="wc-email-settings-table-name"><?php echo __( 'Estrutura da Tabela de Registo', $this->id ); ?></td>
				<td><?php if (!$has_entity_column) echo __( "Faça reload para verificar se o problema foi resolvido.", $this->id );?></td>
			</tr>
			...
			<p>
			<?php _e('CERTIFIQUE-SE QUE:<br>1. tem a livraria SOAP para PHP ativa<br>2. tem a API REST do Woocommerce ativa<br><br>', $this->id); ?></p>
		...
		function GenerateReference($order_id, $order_value)
		{
			...
			if (isset($billing_data['_billing_first_name'][0])) $billing_data['h_client_name'] = $billing_data['_billing_first_name'][0];
